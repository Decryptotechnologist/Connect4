...
public final class Sound implements LineListener {   
    ...
    */    
    public Sound(String filename, double vol){
        ...
        Clip oldSnd = Connect4.sndMap.get(filename);
        if(oldSnd != null){
            this.aClip = oldSnd;
        } else {
            try{
                System.out.println("Loading Sound: "+Connect4.soundCount+": " + audioFileName);
                ...
                setVolume(vol);
                Connect4.sndMap.put(filename, aClip);
                Connect4.soundCount++;
            } catch(Exception e){
            }
        }
    ...
    */    
    public Sound(String filename){
        ...
        AudioClip oldAudio = Connect4.audioMap.get(filename);
        if(oldAudio != null){
            this.clip = oldAudio;
        } else {
            try{
                System.out.println("Loading Audio: "+Connect4.audioCount+": "+"/snd"+ filename);
                clip = Applet.newAudioClip(Sound.class.getResource("/snd"+filename));
                Connect4.audioMap.put(filename, clip);
                Connect4.audioCount++;
            } catch (Exception e) {
            }
        }
    ...
    protected void finalize() throws Throwable {
        if(!fileName.isEmpty()){ 
            Connect4.sndMap.remove(fileName);
            System.out.println("Removing Sound from memory: " + fileName);
        }
        if(!audioFileName.isEmpty()){ 
            Connect4.audioMap.remove(audioFileName);
